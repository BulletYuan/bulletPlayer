...
function bulletPlayer(obj){
    ...
    let CTX=null,
        ...
        _ROTATESCREEN=false,  //对于移动端，全屏已旋转屏幕标识
    ...
    let _initData={
        initSize:function(){
            ...
            let _w2h=0,_c2=0,vw=VIDEO.videoWidth,vh=VIDEO.videoHeight;
            _w2h=vw/vh;
            _c2=CON.clientWidth/CON.clientHeight;
            WIDTH=CON.clientWidth;
            HEIGHT=WIDTH/_w2h;
            if(HEIGHT>=CON.clientHeight){
                HEIGHT=CON.clientHeight;
                WIDTH=HEIGHT*_w2h;
            }
            CAV.height=HEIGHT*QUALITY;
            CAV.width=WIDTH*QUALITY;
            CAV.style.width=WIDTH+"px";
            CAV.style.height=HEIGHT+"px";
            CAV.style.marginTop=((CON.clientHeight-HEIGHT)/2)+"px";
            // let attr=CON.getAttribute('class');
            // CON.setAttribute('class',attr.replace(/rotate/g,''));
            if(_ROTATESCREEN===false&&_FULLSCREEN===1&&window.innerWidth<window.innerHeight&&CTX){
                let attr=CON.getAttribute('class');
                CON.setAttribute('class',`${attr} rotate`);
                CAV.height=HEIGHT*QUALITY;
                CAV.width=WIDTH*QUALITY;
                CAV.style.width=WIDTH+"px";
                CAV.style.height=HEIGHT+"px";
                _ROTATESCREEN=true;
                CAV.style.marginLeft=((CON.clientWidth-WIDTH)/2)+"px";
            }
            if(_ROTATESCREEN===true&&_FULLSCREEN===0&&CTX){
                // CAV.height=HEIGHT*QUALITY;
                // CAV.width=WIDTH*QUALITY;
                // CAV.style.width=WIDTH+"px";
                // CAV.style.height=HEIGHT+"px";
